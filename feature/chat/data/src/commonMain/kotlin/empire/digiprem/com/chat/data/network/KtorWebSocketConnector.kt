...
class KtorWebSocketConnector(
    ...
    private val isInForeground = appLifecycleObserver
        ...
        .onEach { isInForeground ->
            if (isInForeground) {
                connectionRetryHandler.restDelay()
            }
        }
        .stateIn(
            applicationScope,
    ...
    val messages = combine(
        ...
        if (authInfo == null) {
            ...
            createWebSocketFlow(authInfo.accessToken)
                ...
                .retryWhen { t, attempt ->
                    logger.info("Connection failed on attemp $attempt")
                ...
                .catch {e->
                    logger.error("Unhandled WebSocket erro",e)
    ...
    private fun createWebSocketFlow(accessToken: String) = callbackFlow {
        ...
        awaitClose {
            launch(NonCancellable) {
                logger.info("Disconnection from WebSocket session..")
                _connectionState.value = ConnectionState.DISCONNECTED
                currentSession?.close()
                currentSession = null
            }
