...
actual class ConnectionErrorHandler {
    ...
    actual fun transformException(exception: Throwable): Throwable {
        if (exception is CancellationException){
            ...
            val isConnectedLostException=cause.message?.contains("NSURLErrorDomain Code=-1005")==true
            val isNotConnectedException=cause.message?.contains("NSURLErrorDomain Code=-1005")==true

            if (isDarwinException || isConnectedLostException || isNotConnectedException){
    ...
    private  fun Throwable.toNSError():NSError?{
        return message?.let { message->
            when{
                ...
                message.contains(NSURLErrorNotConnectedLostPattern)->
                    return NSError.errorWithDomain(
                        domain = NSURLErrorDomain,
    ...
    companion object{
        private  val NSURLErrorNotConnectedToInternetPattern ="Error Domain=${NSURLErrorDomain} Code=${NSURLErrorNotConnectedToInternet}"
         val NSURLErrorNotConnectedLostPattern
